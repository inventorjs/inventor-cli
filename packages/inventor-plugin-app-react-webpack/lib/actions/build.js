...
export default class BuildAction extends Action {
    ...
    async run(_params, options) {
        ...
        const buildTask = new Promise((resolve, reject) => {
            compiler.run((err, stats) => {
                ...
                if (statJson.warnings?.length) {
                    ...
                    this.log.raw(this.color.yellow(statJson.warnings
                        .filter((item) => !item.message.includes('size limit:'))
                        .map((item) => item.message)
                        .join('\n')));
                }
                this.log.raw(statJson.assets?.map((asset) => {
        ...
        await this.loadingTask(buildTask, {
            ...
            successText: (timeCost) => this.color.green(`webpack build assets successfully[timeCost: ${this.color.yellow(`${timeCost}ms`)}]`),
